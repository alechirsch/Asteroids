		...
		checkInputs: function(){
			...
			if(ig.input.state('up') || (ig.input.state('left') && !ig.input.state('right')) || (ig.input.state('right') && !ig.input.state('left'))){
				...
				if(ig.input.state('left') && !ig.input.state('right')){
					...
					if(stateManager.isArcade() == false)
			...
			if(!moving)
			{
				this.vel.x /= 1.00 + stateManager.gameMode.shipProperties.engineLevel * .002;
				this.vel.y /= 1.00 + stateManager.gameMode.shipProperties.engineLevel * .002;
			}
