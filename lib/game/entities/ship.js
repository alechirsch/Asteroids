...
.defines(function(){
	ShipEntity = ig.Entity.extend({
		...
		thrusterTurn: new ig.Sound('media/thrusters.*'),
		...
		init: function(x, y, settings){
			...
			this.thrusterTurn.loop = true;
			this.thrusterTurn.volume = 0.5;
		},
		...
		checkInputs: function(){
			...
			if(ig.input.state('left') && !ig.input.state('right')){
				...
				//this.thrusterTurn.stop();
			}
			this.currentAnim.angle = this.angle;
			if(ig.input.state('space')){
				if(this.shootTimer.delta() >= 0){
					ig.game.spawnEntity(LaserEntity, this.pos.x * Math.cos(this.angle), this.pos.y * Math.sin(this.angle));
