		...
		checkInputs: function(){
			...
			if(ig.input.state('space')){
				if(this.shootTimer.delta() >= 0){
					...
					var x = this.pos.x;// + (this.size.y/2)*Math.cos(this.angle);
					var y = this.pos.y;// + (this.size.y/2)*Math.sin(this.angle);
					...
					extraX = 0;//Math.sin(this.angle + 3.14) * 80;//Math.cos(this.angle)*32;
					extraY = 0;//Math.cos(this.angle + 3.14) * 80;//Math.sin(this.angle)*32;
					
					//console.log(extraX);
					//console.log(extraY);

					//x = 9 and y = 75
					//laserExtraX = (Math.sin(this.angle) * 75)/2;//-Math.abs(Math.sin(this.angle)*75) - Math.abs(Math.cos(this.angle)*9);
					//laserExtraY = -(Math.cos(this.angle) * 75);//-Math.abs(Math.cos(this.angle)*75) - Math.abs(Math.sin(this.angle)*9);
					...
					laserExtraY -= (laserExtraY - 75);

					ig.game.spawnEntity(LaserEntity, x + inGameWidth/2 + extraX - laserExtraX/2, y + inGameHeight/2 + extraY - laserExtraY/2);
